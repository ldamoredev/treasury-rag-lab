...
import type { CachedEmbeddings, EmbeddingCache } from "./embedding-cache.js";
...
export class JsonEmbeddingCache implements EmbeddingCache {
  ...
  constructor({ filePath, provider, model }: JsonEmbeddingCacheOptions) {
  ...
  async setMany(entries: Map<string, number[]>, dimensions: number): Promise<void> {
    if (entries.size === 0) {
      return;
    }
  ...
  private async load(): Promise<CacheState> {
    try {
      ...
      if (parsed.provider !== this.provider || parsed.model !== this.model) {
        throw new Error(
          `Embedding cache belongs to ${parsed.provider}/${parsed.model}, expected ${this.provider}/${this.model}`,
        );
      }

      for (const [hash, embedding] of Object.entries(parsed.embeddings)) {
        if (embedding.length !== parsed.dimensions) {
          throw new Error(`Cached embedding ${hash} has an unexpected dimension`);
        }
      }
